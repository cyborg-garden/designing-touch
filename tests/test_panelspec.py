...
def test_the_quantised_sliders_are_exactly_the_ones_whose_engine_rounds():
    ...
    guessed. These have integer consumers; everything else in the rack is
    genuinely continuous and must stay continuous — faking coarseness is the
    same lie as faking precision."""
...
def test_a_finely_nudgeable_slider_never_ratchets():
    ...
    must snap plainly — otherwise every sub-pixel write would step it."""
    from dtouch.modes.dithergirl import DitherGirlMode
    from dtouch.panelspec import nudge_unreachable
